...
- Agent Native supersedes Agent Direct as the default runtime-test path. Handoff exports current subject facts, exact Job root, workspace, and advisory environment observations without creating authorization, Session, expiry, capability, revision/origin lease, browser driver, credential rule, action policy, or side-effect scope.
- Native results are linked `agent-native-observation-bundle` artifacts. They record actual revisions/origins/environment/effects and `OBSERVED_EQUIVALENT`, `OBSERVED_MISMATCH`, or `INCONCLUSIVE` without strict-parity claims. Agent Direct remains a backward-compatible explicit audit mode; its existing authorization and attestation contracts are unchanged.
