...
- Agent Direct Test Sessions supersede Workflow-driven exploration for new runtime tests. A private USER authorization binds the exact Job manifest, revisions, origins, equivalent Environment Comparison, capability, and expiry. The Workflow supplies no browser driver, action planner, readiness detector, or executable plan: the local Agent directly chooses its browser/testing tools and may use JavaScript, CSS/XPath, loops, and adaptive decisions within the authorization. Its submitted result is an `AGENT_ATTESTED` claim with hashed private evidence, never Workflow-verified or strict parity.
- `AGENT_DIRECT_READ_ONLY` is enabled in protocol 9. `AGENT_DIRECT_SIDE_EFFECT` is represented by the same schema so future authorization can bind exact systems, object types, action classes, operation budget, test-data prefix, and non-recoverability acceptance, but this Workflow release rejects that capability before platform access.
