...
8. A user-requested retest starts immediately under the Agent's current context and produces a new run linked by `previousRunId`; no Workflow authorization or Session renewal is required. Use `agent-native-list/status` only for archival recovery.
