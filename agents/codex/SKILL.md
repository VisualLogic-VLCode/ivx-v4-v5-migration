...
- Agent Native is the default runtime-test mode. Managed Agent Direct remains available only when the user explicitly asks for the legacy/audit-grade authorization-and-attestation flow; never silently use it for ordinary testing.
...
6. Compare V4/V5 under the best available equivalent business intent. Record what was actually tested, coverage, observed source/target workIds and origins, environment differences, actual effects, console/network/runtime errors, exclusions, and redacted evidence. Facts may drift from the handoff; record them instead of treating them as a lease violation. Never include Token, Cookie, session values, browser storage contents, or other secrets.
7. Submit one Schema-v2 `agent-native-observation-bundle` with `review agent-native-submit --review <id> --file <file>`. Use `OBSERVED_EQUIVALENT`, `OBSERVED_MISMATCH`, or `INCONCLUSIVE`; keep `strictParityClaimed:false` and `workflowRestrictionsApplied:false`. This is an Agent observation, never strict Runtime Parity.
8. A user-requested retest starts immediately under the Agent's current context and produces a new run linked by `previousRunId`; no Workflow authorization or Session renewal is required. Use `agent-native-list/status` only for archival recovery. Use legacy Agent Direct only when the user explicitly requests its managed audit semantics.
