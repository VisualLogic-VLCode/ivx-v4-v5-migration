...
11. Treat first-screen equality only as a smoke baseline while controls, routes, events, or service calls remain discoverable. Inspect the V4/V5 static artifacts plus runtime UI, navigation, and network/service surface; build an Agent-authored candidate-flow inventory for initialization and the discoverable business paths.
12. Classify every candidate as `READ_ONLY`, `WRITE`, or `UNKNOWN` from evidence. Analyze unknown requests/actions instead of blanket-whitelisting them. Execute paired read-only flows; execute writes fully only when the user's request and host policy permit, otherwise stop at and record the pre-submit boundary. Preserve unresolved unknown, blocked, or unexecuted candidates with exact stop reasons.
13. Compare V4/V5 step by step, not by screenshot alone. Record each flow's discovery sources, execution scope, result, step count, stop reason and evidence, plus the derived queue counts, actual effects, errors, exclusions, revisions/origins, and environment facts. Drift is a fact, not a lease violation. Never record Token/Cookie/session values or browser storage contents.
14. Submit a Schema-v2 `agent-native-observation-bundle` with `agent-native-submit`. Initial/user retests use `exploration.scope:WHOLE_CASE`; repair regression uses `AFFECTED_FLOWS`. `OBSERVED_EQUIVALENT` requires the complete five-part inventory, at least one candidate, an exhausted queue, no `UNKNOWN` effect, and only matched full/pre-submit flows. Any remaining unknown/blocked/unexecuted candidate requires `INCONCLUSIVE`; a mismatched candidate supports `OBSERVED_MISMATCH`. Keep `strictParityClaimed:false` and `workflowRestrictionsApplied:false`. Retests link `previousRunId` and repeat this coverage discipline without a new Workflow authorization or Session.
15. For mismatch/useful inconclusive evidence, retrieve candidates and Knowledge, then let the current Agent/LLM classify cause, responsibility, target, confidence, evidence, and used rule ids. Workflow validates policy but must not substitute another semantic cause.
16. Never Patch `CONVERTER`, `PLATFORM_RUNTIME`, `ENVIRONMENT_CONFIGURATION`, `TEST_HARNESS`, `FLAKY_RUNTIME`, `KNOWLEDGE_GAP`, `AUTHORIZATION`, or `UNKNOWN`. Patch only CLI-approved high-confidence `SOURCE_DATA`/`TARGET_CASE` clusters targeting `V5_ARTIFACT`.
17. For authorized repair, create the short-lived INITIAL lease, submit the smallest evidence-linked RFC 6902 proposal including affected Native run ids, require `LOCAL_VALIDATED`, perform one managed CAS/read-back target update, and reconcile uncertainty without replay.
18. Retest autonomously with a linked `REPAIR_REGRESSION` Native bundle and `repairBatchId`. Workflow records verified/failed/inconclusive. Continue only inside initial budgets; stop on repeat, oscillation, growth, regression, flaky evidence, drift, ambiguity, or pause, and request separate `+2/+5` extension authorization.
19. Report Native outcomes as observations rather than strict parity, together with target revision, tested business-flow inventory and coverage, actual effects, unresolved candidates, environment facts, diagnosis/maintainer path, known-issues copy state, and next safe action.
