...
The current release candidate is Workflow `0.8.1` with Agent protocol 9, Converter `1.2.5`, and Knowledge Runtime `0.1.6`. Its capabilities are:
...
- backward-compatible reading and recovery of protocol-8 autonomous-exploration artifacts and commands, while new tests use Agent Direct sessions; current-user ephemeral credentials are allowed only through the exact-subject browser policy, and the separately modeled side-effect capability remains explicitly disabled in 0.8.1;
