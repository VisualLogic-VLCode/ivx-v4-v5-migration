...
The stable `0.4.1` release uses Agent protocol 5 with Converter `1.2.1` and the signed Knowledge Runtime. It provides:
...
After a completed Job has an existing V5 target, Workflow `0.4.1` can create and recover an independent Runtime Review Session from the Job's runtime pins and a revision-checked platform read-back:
