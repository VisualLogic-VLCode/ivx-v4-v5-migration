...
The current signed stable release is Workflow `0.12.1` with Agent protocol 9, Converter `1.2.6`, and Knowledge Runtime `0.1.6`. Its capabilities are:
...
- advisory object/gid preflight, platform-authoritative member permission, source revision checks, resumable Save As checkpoints, source Domain Binding preservation with target-owned route allocation, final nid rewrite, post-save read-back verification, and content-guarded source-revision reconciliation before Runtime Review;
...
Platform writes remain disabled by default. A verified save requires private config `platform.writeMode: "explicit"` and `--confirm-live-write SAVE_V5`. A Job with any fully classified known issue may use the separate command and confirmation `SAVE_V5_WITH_KNOWN_ISSUES`; it finishes as `DIAGNOSTIC_COPY_CREATED`, never `SUCCEEDED`. Classification never bypasses authentication, actual server permission, current platform availability, source-revision safety, or user authorization. Read-only preflight does not infer member write permission from `memberType`, group ownership, or deployment-specific exceptions: the authorized Save As/update endpoint is the authority. A structured platform rejection stops without replay; an ambiguous response remains an unknown outcome and is handled only by read-back/reconciliation.
...
- Read-only preflight proves only object readability/existence and explicit `gid` consistency; it never proves Save As or target-update permission. The real write endpoint decides under the current Token.
- A recognized structured permission rejection is terminal for that write attempt. Generic errors, transport loss, and unrecognized responses remain unknown and are never automatically replayed.
