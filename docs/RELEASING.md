...
Workflow `0.12.2` keeps Agent protocol 9 and the Converter/Knowledge compatibility ranges unchanged. It removes local `memberType`, Group-owner, and deployment-specific membership authorization decisions from Save As and existing-target update preflights. Read-only preflight remains responsible for object access and explicit gid consistency; the real platform write endpoint is authoritative. Endpoint-scoped structured permission rejections are recorded as `REJECTED_BY_PLATFORM` without replay, while transport loss, generic failures, contradictory read-back, and unrecognized responses retain the existing unknown-outcome reconciliation path. Save As, Existing Target Refresh, and Runtime Repair share this distinction; the Agent commands and observation protocol are unchanged.

Workflow `0.12.3` keeps Agent protocol 9, runtime behavior, Schemas, Agent Skills, capabilities, and the Converter/Knowledge compatibility ranges unchanged. It corrects the active stable-version prose bundled in immutable Workflow `0.12.2`; no platform, migration, runtime-test, repair, or update semantics change.

Workflow `0.12.4` keeps Agent protocol 9 and the Converter/Knowledge compatibility ranges unchanged. It makes the CLI-managed Job/Refresh/Review inventories and successful `status`/`recover` the sole authority for local migration state, normalizes a missing Job to `JOB_NOT_FOUND`, exposes `job list` in CLI help, and updates both Agent Skills so workspace planning prose, conversation history, and platform V5 existence cannot substitute for managed lineage or complete a new request.
