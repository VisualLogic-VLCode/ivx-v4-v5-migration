...
> 状态：阶段 0–7 已实现；阶段 8 已完成真实案例验收，Workflow `0.4.3` 正在完成旧用户协调恢复与公开分发验收
...
- 当前实现已把诊断副本改造成“问题类别不否决另存，写入硬前提独立判定”：所有受支持分类均可评估，平台、认证、权限、修订和写结果门禁仍单独强制执行。
...
以下默认方案已由维护者确认，并由阶段 0–7 的合同、策略与回归测试锁定：
...
代码改造阶段已经完成。Knowledge Runtime `0.1.2` 已发布，受控真实案例已完成另存、环境门禁和无人值守运行时一致性验收。Workflow `0.4.0` 的公共全新安装发现 GitHub Release 正文下载在标准 Fetch 上稳定超时，`0.4.1` 增加经哈希/签名门禁约束的系统下载路径；`0.4.2` 为旧运行时提供显式、防降级的一次性 Launcher 恢复，`0.4.3` 将恢复入口明确为协调式 setup，以原子补齐 Knowledge。完成 `0.4.3` 全新安装、旧用户恢复、Agent 同步和回滚验收后阶段 8 关闭。任何尚未以真实稳定场景覆盖的能力都应如实报告，不以静态或 mock 结果替代运行时等价结论。
