...
> 状态：已根据维护者意见修订，待明确开始指令后实施；不代表已实现
...
本文只记录方案。维护者确认前，不修改 Workflow 代码、Converter、平台案例或正式分发通道。
...
- 当前实现只允许 `CONVERTER`、`SOURCE`、`UNKNOWN` 已知问题在用户单独授权后创建诊断副本，而 `PLATFORM` 和 `AUTHORIZATION` 会拒绝另存；目标设计将把这一限制改造成“问题类别不否决另存，写入硬前提独立判定”。
...
维护者明确要求开始改造后，先完成“阶段 0：术语核对与 ADR”，再执行“阶段 1：Schema v2 与向后兼容”。在阶段 2 之前不进行真实平台配置写入；在阶段 5 的只报告试点通过前不开放 AI 自动修改目标案例；在阶段 7 完成并受控验收前不向普通用户宣传“自动修复直到运行时通过”。
