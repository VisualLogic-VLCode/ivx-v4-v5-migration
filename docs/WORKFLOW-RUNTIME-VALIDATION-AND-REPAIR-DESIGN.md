...
> 状态：阶段 0–7 已实现并进入 Workflow `0.4.0` 发布候选；阶段 8 真实案例与公开分发验收进行中
...
代码改造阶段已经完成。当前只剩阶段 8：升级并发布与 Agent protocol 5 兼容的 Knowledge Runtime，执行受控真实案例的另存、环境门禁和运行时链路验收，发布签名 Workflow `0.4.0`，再从公开资产完成全新用户安装、更新、Agent 同步和回滚验收。任何尚未以真实稳定场景覆盖的能力都应如实报告，不以静态或 mock 结果替代运行时等价结论。
