...
> 状态：阶段 0–7 已实现；阶段 8 已完成真实案例验收，Workflow `0.4.1` 正在完成公开分发验收
...
代码改造阶段已经完成。Knowledge Runtime `0.1.2` 已发布，受控真实案例已完成另存、环境门禁和无人值守运行时一致性验收。Workflow `0.4.0` 的公共全新安装发现 GitHub Release 正文下载在标准 Fetch 上稳定超时，因此后续 `0.4.1` 保留全部协议与功能，只增加经哈希/签名门禁约束的系统下载备用路径。完成 `0.4.1` 全新安装、更新、Agent 同步和回滚验收后阶段 8 关闭。任何尚未以真实稳定场景覆盖的能力都应如实报告，不以静态或 mock 结果替代运行时等价结论。
