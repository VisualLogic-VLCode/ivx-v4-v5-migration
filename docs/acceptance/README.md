...
发布前若需要先证明默认不写入、个人/Group 只传 `nid` 的权限预检和脱敏报告，应使用单独的[验收专用 Agent 提示](../templates/AI-AGENT-ACCEPTANCE-PROMPT.md)与[外部普通用户验收清单](../EXTERNAL-USER-ACCEPTANCE.md)。该路径第一阶段故意停在 `READY_TO_SAVE` 或安全停止，不代表普通用户说“转成 V5”时也必须二次确认普通另存。
